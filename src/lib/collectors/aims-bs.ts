...
// Distance Races, aims-worldrunning.org) para Bahamas.
//
// No existe una federación o plataforma bahamesa con calendario propio
// scrapeable (World Athletics lista a la Bahamas Association of
// Athletic Associations pero sin sitio con calendario de carreras de
// calle). AIMS sí trae la carrera de ruta más relevante del país como
// fuente primaria: no es un agregador comercial, es el organismo
...
// por país (/countries/12.html para Bahamas; el ID de país se obtuvo
// navegando /directory.html). Mismo patrón que aims-sa.ts, aims-th.ts,
// aims-vn.ts, aims-ca.ts, aims-jm.ts, aims-tt.ts, etc.
//
// La ciudad no sale de la dirección de contacto del organizador (la de
// Marathon Bahamas figura en Plantation, FL, EE.UU. — es la oficina
// administrativa, no la sede de la carrera, que se corre en Nassau) —
// se hardcodea la ciudad real.
//
// OJO: al momento de escribir este collector, Bahamas tiene un solo
// miembro AIMS (Marathon Bahamas, Half & Relay) y su próxima edición
// figura como "to be confirmed" (sin fecha confirmada todavía), por lo
// que el primer run no va a insertar ninguna carrera — el collector
// queda igual escrito y listo para cuando AIMS confirme la fecha (se
// ejecuta en cada cron, así que la va a levantar sola apenas
// aparezca). Mismo caso ya documentado en aims-sa.ts para Arabia
// Saudita.

const ID_PAIS_AIMS = "12";
const URL_PAIS = `https://aims-worldrunning.org/countries/${ID_PAIS_AIMS}.html`;
const CODIGO_PAIS = "BS";
const FUENTE_TIPO = "aims_bs";
...
export async function correrCollectorAimsBs() {
  return registrarEjecucion(FUENTE_TIPO, async () => {
    const res = await fetch(URL_PAIS, {
      headers: { "User-Agent": "WorldRunnerBot/1.0 (+https://theworldrunner.com)" },
    });
    if (!res.ok) throw new Error(`AIMS (Bahamas) respondió ${res.status}`);
