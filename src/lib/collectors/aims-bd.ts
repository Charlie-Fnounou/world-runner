...
// Distance Races, aims-worldrunning.org) para Bangladesh. AIMS no es un
...
// por país (/countries/146.html para Bangladesh; el ID de país se
// obtuvo una sola vez navegando /directory.html) en vez de tocar el
...
// único dato geográfico que trae la página) — la mayoría de las
// carreras bangladesíes tienen su organizador en Dhaka aunque alguna
// (ej. "Northern Marathon" en Saidpur) esté en otra ciudad, así que es
// un dato aproximado, no siempre la sede exacta. Un caso ("Jolshiri
// RunBangla Marathon") trae la dirección del organizador en EE.UU.: en
// ese caso queda el nombre del país como ciudad para no guardar un dato
// engañoso. La distancia se infiere del propio nombre de la carrera
// ("Marathon", "Half Marathon" o un número final tipo "10"/"21"/"25"),
// sin inventar datos que la página no trae.
//
// Ver también los collectors hermanos aims-th.ts, aims-vn.ts, aims-my.ts,
// aims-lk.ts y aims-np.ts (misma fuente, mismo parser, distinto país).

const ID_PAIS_AIMS = "146";
const URL_PAIS = `https://aims-worldrunning.org/countries/${ID_PAIS_AIMS}.html`;
const PAIS = "Bangladesh";
const CODIGO_PAIS = "BD";
const FUENTE_TIPO = "aims_bd";
...
export async function correrCollectorAimsBd() {
