...
// Distance Races, aims-worldrunning.org) para Túnez. No se encontró un
// calendario público scrapeable de la Fédération Tunisienne d'Athlétisme
// (fta.org.tn no carga o no tiene sección de carreras de calle) ni
// ninguna plataforma tunecina de timing/inscripción con calendario
...
// por país (/countries/92.html para Túnez; el ID de país se obtuvo
// navegando /directory.html) en vez de tocar el calendario
// (/calendar.html) o las tarjetas .ics de /events/, que sí están
// bloqueadas.
//
// Al momento de escribir este collector, Túnez tiene un solo miembro
// AIMS (Marathon COMAR Tunis - Carthage). Mismo patrón que los
// collectors hermanos aims-cn.ts, aims-th.ts, aims-vn.ts, aims-id.ts,
// aims-sg.ts, aims-kr.ts y aims-sa.ts (misma fuente, mismo parser,
// distinto país).
//
// La ciudad se obtiene de la dirección de contacto del organizador (el
// único dato geográfico que trae la página) — para esta carrera es la
// oficina de COMAR (aseguradora patrocinadora) en Túnez capital, así
// que coincide razonablemente con la sede real de la carrera.

const ID_PAIS_AIMS = "92";
const URL_PAIS = `https://aims-worldrunning.org/countries/${ID_PAIS_AIMS}.html`;
const PAIS = "Túnez";
const CODIGO_PAIS = "TN";
const FUENTE_TIPO = "aims_tn";
...
export async function correrCollectorAimsTn() {
