...
// Distance Races, aims-worldrunning.org) para Sri Lanka. AIMS no es un
...
// por país (/countries/86.html para Sri Lanka; el ID de país se obtuvo
// una sola vez navegando /directory.html) en vez de tocar el
// calendario (/calendar.html) o las tarjetas .ics de /events/, que sí
// están bloqueadas.
//
// Sri Lanka tiene un solo miembro de AIMS por ahora (LSR Colombo
// Marathon), pero se deja el mismo parser genérico que el resto de los
// collectors hermanos para que capture nuevas carreras si se dan de
// alta más adelante.
//
// La ciudad se obtiene de la dirección de contacto del organizador (el
// único dato geográfico que trae la página) — suele coincidir con la
// sede real de la carrera, pero no siempre. La distancia se infiere del
// propio nombre de la carrera ("Marathon", "Half Marathon" o un número
// final tipo "10"/"21"), sin inventar datos que la página no trae.
//
// Ver también los collectors hermanos aims-th.ts, aims-vn.ts, aims-my.ts,
// aims-bd.ts y aims-np.ts (misma fuente, mismo parser, distinto país).

const ID_PAIS_AIMS = "86";
const URL_PAIS = `https://aims-worldrunning.org/countries/${ID_PAIS_AIMS}.html`;
const PAIS = "Sri Lanka";
const CODIGO_PAIS = "LK";
const FUENTE_TIPO = "aims_lk";
...
export async function correrCollectorAimsLk() {
