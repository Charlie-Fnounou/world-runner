...
// Distance Races, aims-worldrunning.org) para Panamá.
//
// No se encontró una federación o plataforma panameña con calendario
// propio scrapeable en el tiempo asignado a esta fuente. AIMS sí trae
// las dos carreras de ruta más relevantes del país como fuente
...
// por país (/countries/71.html para Panamá; el ID de país se obtuvo
// navegando /directory.html). Mismo patrón que aims-sa.ts, aims-th.ts,
// aims-vn.ts, aims-ca.ts, aims-jm.ts, aims-tt.ts, aims-bs.ts, etc.

const ID_PAIS_AIMS = "71";
const URL_PAIS = `https://aims-worldrunning.org/countries/${ID_PAIS_AIMS}.html`;
const CODIGO_PAIS = "PA";
const FUENTE_TIPO = "aims_pa";

const CIUDADES: Record<string, string> = {
  "10504": "Ciudad de Panamá",
  "932": "Ciudad de Panamá",
};
...
export async function correrCollectorAimsPa() {
  return registrarEjecucion(FUENTE_TIPO, async () => {
    const res = await fetch(URL_PAIS, {
      headers: { "User-Agent": "WorldRunnerBot/1.0 (+https://theworldrunner.com)" },
    });
    if (!res.ok) throw new Error(`AIMS (Panamá) respondió ${res.status}`);
