...
// Distance Races, aims-worldrunning.org) para China. Se intentó antes
// con runchina.org.cn (SPA vacía en el HTML servido, y su calendario
// da error 567 — bloqueo explícito de bots) y no había forma de
// scrapearlo sin JavaScript. AIMS es una alternativa legítima como
...
// por país (/countries/22.html para China; el ID de país se obtuvo
...
// único dato geográfico que trae la página) — en varios casos es la
// sede administrativa del organizador y no la ciudad exacta de la
// carrera (ej. "Chengdu Marathon" queda con ciudad "Chaoyang District,
// Beijing" porque ahí tiene oficina la agencia organizadora), así que
// es un dato aproximado, no la sede exacta. La distancia se infiere
// del propio nombre de la carrera ("Marathon", "Half Marathon" o un
// número final), sin inventar datos que la página no trae.
//
// Ver también los collectors hermanos aims-th.ts, aims-vn.ts,
// aims-id.ts, aims-sg.ts y aims-kr.ts (misma fuente, mismo parser,
// distinto país).

const ID_PAIS_AIMS = "22";
const URL_PAIS = `https://aims-worldrunning.org/countries/${ID_PAIS_AIMS}.html`;
const PAIS = "China";
const CODIGO_PAIS = "CN";
const FUENTE_TIPO = "aims_cn";
...
export async function correrCollectorAimsCn() {
